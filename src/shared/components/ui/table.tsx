...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(({ className, ...props }, ref) => (
  <tr
    ref={ref}
    className={cn("data-[state=selected]:bg-muted hover:bg-muted/50 border-slate-600 border-b transition-colors", className)}
    {...props}
  />
));
...
const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn("px-2 h-10 font-medium text-left align-middle whitespace-nowrap bg-[rgba(30,45,55,0.90)] backdrop-blur-sm [&>[role=checkbox]]:translate-y-[2px]", className)}
    {...props}
  />
));
