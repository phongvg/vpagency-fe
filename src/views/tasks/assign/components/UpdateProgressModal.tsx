import { Button, ConfirmDialog, DatePicker, Dialog, Input } from '@/components/ui'
...
export default function UpdateProgressModal({ isOpen, taskId, onClose }: Props) {
  ...
  const { showConfirm, confirmProps } = useConfirmDialog({
    title: 'Xác nhận lưu thông tin URL',
    type: 'info',
    cancelText: 'Hủy',
  })
  ...
  const handleAssignItem = async (finalUrlId: string, itemIndex: number) => {
    ...
    const confirmed = await showConfirm({
      message: 'Bạn có chắc chắn muốn lưu thông tin URL?',
    })
  ...
  return (
    <>
                    ...
                    >
                      ...
                      <div className="px-4 py-3 text-sm">
                        <DatePicker
                          ...
                          disabled={isItemAssigned(group.finalUrlId, itemIndex)}
                      ...
                      <div className="px-4 py-3 text-sm">
                        <SelectCustom
                          ...
                          isDisabled={!item.date || isItemAssigned(group.finalUrlId, itemIndex)}
                          onChange={(uid: any) => {
      ...
      <ConfirmDialog {...confirmProps} loading={assignMutation.isPending} />
    </>
  )
}
