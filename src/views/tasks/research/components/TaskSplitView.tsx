...
export default function TaskSplitView() {
  ...
  const { selectedTask, setSelectedTask, filters, setFilters } = useResearchStore()
  const { data: taskList } = useMockData()
  const { data: taskDetail, isFetching: isLoadingTask } = useMockDetail(id)
  const [isMobileDetailView, setIsMobileDetailView] = useState(false)

  const handleTaskSelect = (task: Research) => {
...
interface TaskListPanelProps {
  ...
  onTaskSelect: (task: Research) => void
  pagination: {
    total: number
    currentPage: number
    pageSize: number
    ...
    >
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className={`px-2 py-0.5 rounded-full font-medium ${difficultyColors[task.difficulty]}`}>
            {difficultyLabels[task.difficulty]}
          </span>
          {task.result && <span className="text-green-600">Có kết quả</span>}
        </div>
      </div>
    </div>
  )
