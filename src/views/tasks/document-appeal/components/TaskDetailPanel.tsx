...
import { HiCalendar, HiCheckCircle, HiClipboardList, HiCube, HiPencil } from 'react-icons/hi'
...
export default function TaskDetailPanel() {
  const { selectedTask, openDialog } = useDocumentAppealStore()

  if (!selectedTask) return null

  const task = selectedTask as DocumentAppeal
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-purple-50 p-4 rounded-lg md:col-span-2">
            <div className="flex items-center gap-2 text-purple-700 mb-2">
              ...
              <span className="text-sm font-medium">Nền tảng</span>
            </div>
            <p className="text-purple-900 font-semibold">{task.platform}</p>
          </div>
