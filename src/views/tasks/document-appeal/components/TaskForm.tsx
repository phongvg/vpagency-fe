...
import { CreateDocumentAppealRequest, DocumentAppeal } from '../types/documentAppeal.type'

type Props = {
  task?: DocumentAppeal | null
  isEdit?: boolean
  loading?: boolean
  onSubmit: (values: CreateDocumentAppealRequest) => void
  onCancel: () => void
}
...
export default function TaskForm({ task, isEdit = false, loading = false, onSubmit, onCancel }: Props) {
  ...
  const initialValues: CreateDocumentAppealRequest = {
    name: task?.name || '',
    ...
    <div className="task-form">
      <div className="mb-3">
        <h4 className="font-semibold text-gray-900 text-lg">
          {isEdit ? 'Chỉnh sửa kháng giấy tờ' : 'Thêm kháng giấy tờ mới'}
        </h4>
      </div>
          ...
          <Form>
            <div className="gap-4 grid grid-cols-1 lg:grid-cols-2">
              ...
              <FormItem
                ...
                invalid={touched.platform && Boolean(errors.platform)}
                label="Nền tảng"
              >
                <Input
                  name="platform"
                  placeholder="VD: Facebook, Google Ads..."
