...
export default function TaskSplitView() {
  ...
  const { selectedTask, setSelectedTask, filters, setFilters } = useAccountAppealStore()
  const { data: taskList } = useMockData()
  const { data: taskDetail, isFetching: isLoadingTask } = useMockDetail(id)
  const [isMobileDetailView, setIsMobileDetailView] = useState(false)

  const handleTaskSelect = (task: AccountAppeal) => {
...
interface TaskListPanelProps {
  ...
  onTaskSelect: (task: AccountAppeal) => void
  pagination: {
    total: number
    currentPage: number
    pageSize: number
