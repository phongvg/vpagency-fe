import type { Campaign, CurrencyRate, UpdateCampaignRequest } from "@/modules/campaign/types/campaign.type";
import { excelDateToJSDate, getSheetData } from "@/modules/campaign/utils/campaign.util";
  ...
  try {
    ...
    const campaigns: UpdateCampaignRequest[] = uniqueCampaignIds.map((id, index) => {
      ...
      const filteredPfmRows = campaignPfmRows.filter((row) => {
        ...
        const normalizedPfmDate = typeof pfmDatePulled === "number" ? excelDateToJSDate(pfmDatePulled) : String(pfmDatePulled);
      ...
      const criterionMap = new Map(adGroupCriterionRows.map((row) => [`${row["Keyword Text"]}|${row["Keyword Match Type"].toLowerCase()}`, row]));
      ...
      const negativeKeywords = Array.from(new Map(negativeKeywordsRaw.map((k) => [`${k.keyword}|${k.match}`, k])).values());
      ...
      const topSearchTerms = Array.from(
        new Map(topSearchTermsRaw.map((s) => [`${s.term}|${s.cpc}|${s.cost}|${s.clicks}|${s.ctr}|${s.cpm}|${s.impression}`, s])).values()
      );
      ...
      const locationStats = Array.from(
        new Map(locationStatsRaw.map((loc) => [`${loc.location}|${loc.clicks}|${loc.ctr}|${loc.cpc}|${loc.cost}`, loc])).values()
      );
      ...
      return {
        ...
        finalUrl: finalUrlKey[finalUrlKey.length - 1] ? finalUrlKey[finalUrlKey.length - 1] : finalUrlAds[finalUrlAds.length - 1],
        keywords,
        topSearchTerms,
        ...
        targetLocations,
        locationStats,
        campaignBudget: campaignBudget[campaignBudget.length - 1] ? campaignBudget[campaignBudget.length - 1] / 1000000 : 0,
        negativeKeywords,
        locationExcluded,
